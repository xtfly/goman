...
type ArticleComments struct {
	...
	Article *Article `json:"article_id" orm:"index;rel(one)"` //
...
type ArticleVote struct {
	Id         int64    `json:"id" orm:"pk;auto"`
	Uid        int64    `json:"uid" orm:"index"`                                    // 用户ID
	Article    *Article `json:"article_id" orm:"index;rel(one);column(article_id)"` //
	ArticleUid int64    `json:"article_uid" orm:"index"`                            //
