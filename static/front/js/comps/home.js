$(function() {
  ...
  $('#bp_more').click(function() {
    ...
    switch (window.location.hash) {
      default: if (window.location.hash != '#all') {
      ...
      var request_url = '/api/home/indexactions/page-' + $(this).attr('data-page') + '__filter-' + cur_filter;
      break;

      case '#draft_list__draft':
          var request_url = '/api/home/draft/page-' + $(this).attr('data-page');
        if ($('#main_title .btn-success').length <= 0) {
          $('#main_title').prepend('<a class="pull-right btn btn-mini btn-success" onclick="GOM.User.delete_draft(\'\', \'clean\');">' + _t('清空所有') + '</a>');
        }
        break;

      case '#invite_list__invite':
          var request_url = G_BASE_URL + '/api/home/invite/page-' + $(this).attr('data-page');
        break;

      case '#focus_topic__focus':
          var request_url = G_BASE_URL + '/api/home/focustopicslist/page-' + $(this).attr('data-page');
        break;
    }
