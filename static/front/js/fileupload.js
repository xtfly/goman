...
FileUpload.prototype = {
  ...
  createInput: function() {
    var _this = this,
      input = this.toElement('<input type="file" />');
  ...
  upload: function(file, li) {
    ...
    if (file) {
      ...
      var url = this.url + '&upload_file=' + file.name + '&timestamp=' + new Date().getTime();
      xhr.open("POST", url);
      xhr.send(file);
    } else {
      //低版本ie上传
  ...
  oncomplete: function(xhr, li, file) {
    var _this = this,
      response, filesize = this.getFIleSize(file);
  ...
  createInsertBtn: function(attach_id) {
    var btn = this.toElement(this.options.insertBtnTemplate),
      _this = this;
