var GOM = {
  ...
  loading: function(type) {
    ...
    if (type == 'show') {
      ...
      GOM.G.loading_timer = setInterval(function() {
        GOM.G.loading_bg_count -= 1;

        $('#aw-loading-box').css('background-position', '0px ' + GOM.G.loading_bg_count * 40 + 'px');

        if (GOM.G.loading_bg_count == 1) {
          GOM.G.loading_bg_count = 12;
        }
      }, 100);
    } else {
      $('#aw-loading').fadeOut();

      clearInterval(GOM.G.loading_timer);
    }
  },
  ...
  loading_mini: function(selector, type) {
    ...
    if (type == 'show') {
      ...
      GOM.G.loading_timer = setInterval(function() {
        GOM.G.loading_mini_bg_count -= 1;

        $('#aw-loading-mini-box').css('background-position', '0px ' + GOM.G.loading_mini_bg_count * 16 + 'px');

        if (GOM.G.loading_mini_bg_count == 1) {
          GOM.G.loading_mini_bg_count = 9;
        }
      }, 100);
    } else {
  ...
  ajax_post: function(formEl, processer, type) // 表单对象，用 jQuery 获取，回调函数名
    {
      ...
      if (typeof(processer) != 'function') {
        var processer = GOM.ajax_processer;
        GOM.loading('show');
      }
  ...
  ajax_processer: function(type, result) {
    ...
    if (typeof(result.errno) == 'undefined') {
      ...
      if (type == 'comments_form') {
        GOM.reload_comments_list(result.rsm.item_id, result.rsm.item_id, result.rsm.type_name);
  ...
  dialog: function(type, data, callback) {
    ...
    if (template) {
      ...
      switch (type) {
        case 'redirect':
          GOM.Dropdown.bind_dropdown_list($('.aw-question-redirect-box #question-input'), 'redirect');
          break;

        case 'inbox':
          GOM.Dropdown.bind_dropdown_list($('.aw-inbox #invite-input'), 'inbox');
          //私信用户下拉点击事件
        ...
        case 'publish':
          ...
          GOM.Init.init_topic_edit_box('#quick_publish .aw-edit-topic');
  ...
  show_card_box: function(selector, type, time) //selector -> .aw-user-name/.topic-tag
    {
      ...
      $(document).on('mouseover', selector, function() {
        clearTimeout(GOM.G.card_box_hide_timer);
        var _this = $(this);
        GOM.G.card_box_show_timer = setTimeout(function() {
          //判断用户id or 话题id 是否存在
          if (_this.attr('data-id')) {
            switch (type) {
              case 'user':
                //检查是否有缓存
                if (GOM.G.cashUserData.length == 0) {
              ...
              case 'topic':
                //检查是否有缓存
                if (GOM.G.cashTopicData.length == 0) {
          ...
          function _getdata(type, url) {
            if (type == 'user') {
              $.get(url + _this.attr('data-id'), function(result) {
                ...
                _init();
                //缓存
                GOM.G.cashUserData.push($('#aw-ajax-box').html());
              }, 'json');
            }
            if (type == 'topic') {
              $.get(url + _this.attr('data-id'), function(result) {
                ...
                _init();
                //缓存
                GOM.G.cashTopicData.push($('#aw-ajax-box').html());
              }, 'json');
            }
          }
          ...
          function _checkcash(type) {
            if (type == 'user') {
              $.each(GOM.G.cashUserData, function(i, a) {
            ...
            if (type == 'topic') {
              ...
              $.each(GOM.G.cashTopicData, function(i, a) {
                if (a.match('data-id="' + _this.attr('data-id') + '"')) {
                  $('#aw-ajax-box').html(a);
                  $('#aw-card-tips').removeAttr('style');
                  _init();
      ...
      $(document).on('mouseout', selector, function() {
        clearTimeout(GOM.G.card_box_show_timer);
        GOM.G.card_box_hide_timer = setTimeout(function() {
...
GOM.User = {
  ...
  invite_user: function(selector, img) {
    $.post('/question/ajax/save_invite/', {
      ...
      if (result.errno != -1) {
        ...
        selector.addClass('active').attr('onclick', 'GOM.User.disinvite_user($(this))').text('取消邀请');
  ...
  disinvite_user: function(selector) {
    $.get('/question/ajax/cancel_question_invite/question_id-' + QUESTION_ID + "__recipients_uid-" + selector.attr('data-id'), function(result) {
      if (result.errno != -1) {
        ...
        selector.removeClass('active').attr('onclick', 'GOM.User.invite_user($(this),$(this).parents(\'li\').find(\'img\').attr(\'src\'))').text('邀请');
        selector.parents('.aw-question-detail').find('.aw-invite-replay .badge').text(parseInt(selector.parents('.aw-question-detail').find('.aw-invite-replay .badge').text()) - 1);
  ...
  save_comment: function(selector) {
    ...
    GOM.ajax_post(selector.parents('form'), GOM.ajax_processer, 'comments_form');
  },
...
GOM.Dropdown = {
    ...
    $(selector).keyup(function(e) {
      ...
      if ($(selector).val().length >= 1) {
        if (e.which != 38 && e.which != 40 && e.which != 188 && e.which != 13) {
          GOM.Dropdown.get_dropdown_list($(this), type, $(selector).val());
        }
      } else {
  ...
  get_dropdown_list: function(selector, type, data) {
    if (GOM.G.dropdown_list_xhr != '') {
      GOM.G.dropdown_list_xhr.abort(); // 中止上一次ajax请求
    }
    var url;
    switch (type) {
      case 'search':
    ...
    GOM.G.dropdown_list_xhr = $.get(url, function(result) {
      if (result.length != 0 && GOM.G.dropdown_list_xhr != undefined) {
...
GOM.Message = {
  ...
  check_notifications: function() {
    // 检测登录状态
    if (G_USER_ID == 0) {
      clearInterval(GOM.G.notification_timer);
      return false;
    }
    ...
    $.get('/home/ajax/notifications/', function(result) {
      ...
      if (G_UNREAD_NOTIFICATION > 0) {
        if (G_UNREAD_NOTIFICATION != last_unread_notification) {
          // 加载消息列表
          GOM.Message.load_notification_list();
  ...
  read_notification: function(selector, notification_id, reload) {
    ...
    $.get(url, function(result) {
      GOM.Message.check_notifications();
  ...
  init_comment_box: function(selector) {
    $(document).on('click', selector, function() {
      ...
      if ($(comment_box_id).length) {
        ...
        GOM.at_user_lists(comment_box_id + ' .aw-comment-txt', 5);
      }

      GOM.at_user_lists($(this).parents('.aw-item').find('.aw-comment-txt'));
    });
  },
  ...
  init_topic_edit_box: function(selector) //selector -> .aw-edit-topic
    {
      $(selector).click(function() {
        ...
        if (_topic_editor.find('.aw-edit-topic-box').length == 0) {
          ...
          GOM.Dropdown.bind_dropdown_list($(this).parents('.aw-topic-bar').find('#aw_edit_topic_title'), 'topic');
        }
...
function _t(string, replace) {
  if (typeof(GOM_lang) != 'undefined') {
    if (typeof(GOM_lang[string]) != 'undefined') {
      string = GOM_lang[string];
    }
  }
