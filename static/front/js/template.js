var AW_TEMPLATE = {
			...
			'<div id="aw-card-tips" class="aw-card-tips aw-card-tips-user">'+
				'<div class="aw-mod">'+
					...
					'<div class="mod-footer clearfix">'+
						'<span>'+
							'<a class="text-color-999" onclick="GOM.dialog(\'inbox\', \'{{user_name}}\');"><i class="icon icon-inbox"></i> ' + _t('私信') + '</a>&nbsp;&nbsp;&nbsp;&nbsp;<a  class="text-color-999" onclick="GOM.dialog(\'publish\', {category_enable:{{category_enable}}, ask_user_id:{{uid}}, ask_user_name:{{ask_name}} });"><i class="icon icon-at"></i> ' + _t('问Ta') + '</a>'+
						'</span>'+
						'<a class="btn btn-normal btn-success follow {{focus}} pull-right" onclick="GOM.User.follow($(this), \'user\', {{uid}});"><span>{{focusTxt}}</span> <em>|</em> <b>{{fansCount}}</b></a>'+
					'</div>'+
				'</div>'+
			'</div>',
			...
			'<div id="aw-card-tips" class="aw-card-tips aw-card-tips-topic">'+
				'<div class="aw-mod">'+
					...
					'<div class="mod-footer">'+
						...
						'<a class="btn btn-normal btn-success follow {{focus}} pull-right" onclick="GOM.User.follow($(this), \'topic\', {{topic_id}});"><span>{{focusTxt}}</span> <em>|</em> <b>{{focus_count}}</b></a>'+
					'</div>'+
				'</div>'+
			'</div>',
	...
	'editCommentBox' :
				...
				'<div class="modal-dialog">'+
					'<div class="modal-content">'+
						...
						'</div>'+
						'<form action="' + '/question/ajax/update_answer/answer_id-{{answer_id}}" method="post" onsubmit="return false" id="answer_edit">'+
						'<div class="modal-body">'+
						...
						'<div class="modal-footer">'+
							...
							'<button class="btn btn-large btn-success" onclick="GOM.ajax_post($(\'#answer_edit\'), GOM.ajax_processer, \'ajax_post_alert\');return false;">' + _t('确定') + '</button>'+
						'</div>'+
						'</form>'+
					'</div>'+
				'</div>'+
	...
	'articleCommentBox' :
		'<div class="aw-article-replay-box clearfix">'+
			'<form action="'+ '/article/ajax/save_comment/" onsubmit="return false;" method="post">'+
				'<div class="mod-body">'+
				...
				'</div>'+
				'<div class="mod-footer">'+
					'<a href="javascript:;" onclick="GOM.ajax_post($(this).parents(\'form\'));" class="btn btn-normal btn-success pull-right btn-submit">' + _t('回复') + '</a>'+
				'</div>'+
			'</form>'+
		'</div>',
		...
		'<div class="modal hide fade alert-box aw-favorite-box">'+
			'<div class="modal-dialog">'+
				'<div class="modal-content">'+
					...
					'</div>'+
					'<form id="favorite_form" action="' + '/favorite/ajax/update_favorite_tag/" method="post" onsubmit="return false;">'+
						...
						'<div class="mod aw-favorite-tag-add hide">'+
							...
							'<div class="modal-footer">'+
								...
								'<button href="javascript:;" class="btn btn-large btn-success" onclick="GOM.User.add_favorite_tag()">' + _t('确认创建') + '</button>'+
							'</div>'+
						'</div>'+
					'</form>'+
				'</div>'+
			...
			'<div class="modal fade alert-box aw-publish-box">'+
				'<div class="modal-dialog">'+
					'<div class="modal-content">'+
						...
						'<div class="modal-body">'+
							'<div class="alert alert-danger hide error_message"><i class="icon icon-delete"></i> <em></em></div>'+
							'<form action="' + '/publish/ajax/publish_question/" method="post" id="quick_publish" onsubmit="return false">'+
								...
								'<div class="clearfix hide" id="quick_publish_captcha">'+
									...
									'<img id="qp_captcha" class="pull-left" onclick="this.src = \'' +'/account/captcha/\' + Math.floor(Math.random() * 10000);" src="" />'+
								'</div>'+
							'</form>'+
						'</div>'+
						'<div class="modal-footer">'+
							'<span class="pull-right">'+
								...
								'<button class="btn btn-large btn-success" onclick="GOM.ajax_post($(\'#quick_publish\'), GOM.ajax_processer, \'error_message\');">' + _t('发起') + '</button>'+
							'</span>'+
							'<a href="javascript:;" tabindex="-1" onclick="$(\'form#quick_publish\').attr(\'action\', \'' + '/publish/\');$.each($(\'#quick_publish textarea\'), function (i, e){if ($(this).val() == $(this).attr(\'placeholder\')){$(this).val(\'\');}});document.getElementById(\'quick_publish\').submit();" class="pull-left">' + _t('高级模式') + '</a>'+
						'</div>'+
					'</div>'+
				'</div>'+
			'</div>',
			...
			'<div class="modal fade alert-box aw-inbox">'+
				'<div class="modal-dialog">'+
					'<div class="modal-content">'+
						...
						'<div class="modal-body">'+
							'<div class="alert alert-danger hide error_message"> <i class="icon icon-delete"></i> <em></em></div>'+
							'<form action="' + '/inbox/ajax/send/" method="post" id="quick_publish" onsubmit="return false">'+
								'<input type="hidden" name="_csrf" value="' + CSRF_TOKEN + '" />'+
						...
						'<div class="modal-footer">'+
							'<a data-dismiss="modal" aria-hidden="true" class="text-color-999">' + _t('取消') + '</a>'+
							'<button class="btn btn-large btn-success" onclick="GOM.ajax_post($(\'#quick_publish\'), GOM.ajax_processer, \'error_message\');">' + _t('发送') + '</button>'+
						'</div>'+
					'</div>'+
				'</div>'+
			'</div>',
				...
				'<form action="{{comment_form_action}}" method="post" onsubmit="return false">'+
					'<div class="aw-comment-box-main">'+
						...
						'<div class="aw-comment-box-btn">'+
							'<span class="pull-right">'+
								'<a href="javascript:;" class="btn btn-mini btn-success" onclick="GOM.User.save_comment($(this));">' + _t('评论') + '</a>'+
			...
			'<div class="modal fade alert-box aw-share-box aw-share-box-message aw-report-box" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">'+
				'<div class="modal-dialog">'+
					'<div class="modal-content">'+
						...
						'</div>'+
						'<form id="quick_publish" method="post" action="' + '/question/ajax/save_report/">'+
							...
							'<div class="modal-footer">'+
								'<a data-dismiss="modal" aria-hidden="true" class="text-color-999">' + _t('取消') + '</a>'+
								'<button class="btn btn-large btn-success" onclick="GOM.ajax_post($(\'#quick_publish\'), GOM.ajax_processer, \'error_message\');return false;">' + _t('提交') + '</button>'+
							'</div>'+
						'</form>'+
					'</div>'+
				'</div>'+
		...
		'<div class="modal fade alert-box aw-recommend-box">'+
			'<div class="modal-dialog">'+
				'<div class="modal-content">'+
					...
					'</div>'+
					'<form id="help_form" action="' + '/help/ajax/add_data/" method="post" onsubmit="return false;">'+
					'<input type="hidden" name="item_id" value="{{item_id}}" />'+
	...
	'questionRedirectList' :
		'<li class="question"><a class="aw-hide-txt" onclick="GOM.ajax_request({{url}})">{{name}}</a></li>',
				...
				'<div class="modal-dialog">'+
					'<div class="modal-content">'+
						'<div class="formBox">'+
							...
							'<div class="main ">'+
								'<form class="form-horizontal" action="' + '/project/ajax/add_product_order/" onsubmit="return false" role="form" id="projectEventForm" method="post">'+
							...
							'<div class="footer pull-right">'+
								'<a onclick="GOM.ajax_post($(\'#projectEventForm\'));">确定</a>'+
							'</div>'+
						'</div>'+
					'</div>'+
				'</div>'+
